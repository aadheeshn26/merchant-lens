...
export const loader = async () => {
  try {
    const [totalSales, sentiment, recommendations, seo] = await Promise.all([
      ...
      fetch("http://localhost:8000/seo/Mug").then(res => res.json()) // Example product
    ]);
    return json({ totalSales, sentiment, recommendations, seo, error: null });
  } catch (e) {
    return json({ totalSales: { total_sales: 0 }, sentiment: {}, recommendations: { recommendations: [], pricing_suggestions: {} }, seo: {}, error: "Failed to fetch data from backend" });
  }
};
...
export default function Index() {
  const { totalSales, sentiment, recommendations, seo, error } = useLoaderData<typeof loader>();
    ...
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md col-span-1 md:col-span-2 lg:col-span-3">
          <h2 className="text-xl font-semibold text-gray-700 mb-2">SEO Suggestions (Mug)</h2>
          <p className="text-gray-600"><strong>Title:</strong> {seo.title}</p>
          <p className="text-gray-600"><strong>Description:</strong> {seo.description}</p>
        </div>
      </div>
    </div>
  );
